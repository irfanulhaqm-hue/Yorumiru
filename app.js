...
const MEDIA_QUERY=`query($id:Int){Media(id:$id,type:ANIME){id title{romaji english} format coverImage{large} bannerImage description episodes duration seasonYear averageScore genres status nextAiringEpisode{episode airingAt} airingSchedule(notYetAired:false,perPage:1,sort:TIME_DESC){nodes{episode airingAt}} characters(sort:ROLE,perPage:12){edges{node{id name{full} image{large}}}} relations{edges{relationType node{id title{romaji english} format coverImage{large} bannerImage description episodes duration seasonYear status nextAiringEpisode{episode airingAt} airingSchedule(notYetAired:false,perPage:1,sort:TIME_DESC){nodes{episode airingAt}}}}} recommendations(perPage:6){nodes{mediaRecommendation{id title{romaji english} coverImage{large} episodes seasonYear}}}}}`;
async function mediaById(id){try{const r=await fetch(API,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({query:MEDIA_QUERY,variables:{id:Number(id)}})});return (await r.json()).data?.Media||null}catch{return null}}
async function collectSeries(start){
  const seen=new Map();
  const visit=async id=>{if(!id||seen.has(Number(id)))return;const node=Number(id)===Number(start.id)?start:await mediaById(id);if(!node)return;seen.set(node.id,node);for(const e of node.relations?.edges||[]){if(["PREQUEL","SEQUEL"].includes(e.relationType))await visit(e.node?.id)}};
  await visit(start.id);
  const all=[...seen.values()];
  const main=all.filter(n=>["TV","TV_SHORT"].includes(n.format));
  const candidates=main.length?main:all;
  candidates.sort((a,b)=>(Number(a.seasonYear)||9999)-(Number(b.seasonYear)||9999)||a.id-b.id);
  return candidates;
}
function seasonName(n,i){const t=title(n);const m=t.match(/season\s*(\d+)/i);if(m)return `Season ${m[1]}`;if(/final season/i.test(t))return `Season ${i+1} — The Final Season`;return `Season ${i+1}`}
async function openAnime(m){
  ...
  document.querySelectorAll(".season").forEach(s=>s.onclick=()=>openEpisodes(seasons[+s.dataset.s].data,+s.dataset.s,seasons));
  document.querySelectorAll(".sim").forEach(s=>s.onclick=async()=>{const x=await mediaById(s.dataset.id);if(x){animeDlg.close();openAnime(x)}});
  animeDlg.showModal()
}
